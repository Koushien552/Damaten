...
<p class="muted"><b style="color:#185FA5">● 青</b> 上下をつなぐ ／ <b style="color:#E24B4A">● 赤</b> 左右をつなぐ。空マスをクリックで着手。</p>
<script>
...
let board,history,winner,winSet,human,engine,busy;
...
function render(){while(svg.firstChild)svg.removeChild(svg.firstChild);
 ...
 for(let r=0;r<N;r++)for(let c=0;c<N;c++){const x=cx(r,c),y=cy(r),v=board[ix(r,c)];
  const po=document.createElementNS(NS,"polygon");po.setAttribute("points",pts(x,y));po.setAttribute("fill","#fff");po.setAttribute("stroke","#9a988f");po.setAttribute("stroke-width","1");po.style.cursor=(v||winner||busy)?"default":"pointer";po.onclick=()=>click(r,c);svg.appendChild(po);
  if(v){const st=document.createElementNS(NS,"circle");st.setAttribute("cx",x);st.setAttribute("cy",y);st.setAttribute("r",s*0.62);st.setAttribute("fill",v===BLUE?"#185FA5":"#E24B4A");if(winSet&&winSet.has(ix(r,c))){st.setAttribute("stroke","#EF9F27");st.setAttribute("stroke-width","3");}st.style.pointerEvents="none";svg.appendChild(st);}}}
function setStatus(t){statusEl.textContent=t;}
function place(i,col){board[i]=col;history.push(i);movesEl.textContent="手数 "+history.length;}
...
async function engineMove(last){busy=true;setStatus("AI思考中…");render();
 try{const res=await fetch("/move",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({board:board.join(""),player:engine,last:last,iters:+itEl.value})});
  ...
  place(d.move,engine);if(!checkEnd(engine)){setStatus("手番: あなた");}busy=false;render();
...
function newGame(){human=+sideSel.value;engine=human===BLUE?RED:BLUE;board=new Array(N*N).fill(0);history=[];winner=0;winSet=null;busy=false;
 setStatus("手番: あなた");movesEl.textContent="手数 0";render();if(engine===BLUE)engineMove(-1);}
function undo(){if(busy||!history.length)return;winner=0;winSet=null;
 ...
 const side=(history.length%2===0)?BLUE:RED;setStatus("手番: あなた");render();
 if(side===engine&&history.length===0&&engine===BLUE)engineMove(-1);}
